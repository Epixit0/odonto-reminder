 ...
 * @param {string} [options.populate] - Path a popular (ej: "patientId")
 * @param {string} [options.select] - Campos a seleccionar (ej: "name phone tags")
 * @returns {{ items: Array, nextCursor: string|null, hasMore: boolean }}
 */
export async function paginate(model, query = {}, { cursor, limit = 20, sort = { createdAt: -1 }, populate, select } = {}) {
  ...
  if (populate) {
    dbQuery = dbQuery.populate(populate, select);
  }
